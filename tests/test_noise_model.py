...
def test_noise_model_context_switching(mock_rng, default_config):
    ...
    safe_noises = np.array([model.sample_noise(dist_to_obstacle=10.0) for _ in range(100)])
    assert np.mean(np.abs(safe_noises)) < 0.05

    danger_noises = np.array([model.sample_noise(dist_to_obstacle=1.0) for _ in range(100)])
    assert np.any(np.abs(danger_noises) > 0.1)


@pytest.mark.parametrize("distance, expected_safe", [
    (3.01, True),  # Safe zone
...
def test_noise_model_boundary_switching(distance, expected_safe, default_config):
    ...
    samples = np.array([model.sample_noise(dist_to_obstacle=distance) for _ in range(50)])
